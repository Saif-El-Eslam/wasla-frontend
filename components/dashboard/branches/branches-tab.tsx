...
import { api, type BranchManagement, type OpeningHours } from '@/api';
import {
  Badge,
  Card,
  FormPanel,
...
import { useBranchManagement } from '@/hooks/use-venue';
import { queryKeys } from '@/lib/query-keys';
import { textForLocale } from '@/lib/localized-text';
import { useForm } from 'react-hook-form';
import { zodResolver } from '@hookform/resolvers/zod';
import { branchSchema, type BranchFormInput, type BranchFormValues } from '@/schemas/branch.schema';
...
export function BranchesTab({
  ...
  const invalidate = () => void queryClient.invalidateQueries({ queryKey: queryKeys.branches });
            ...
            <Card key={branch.id} className={branch.isMain ? 'relative border-teal-200' : 'relative'}>
              <div className="flex items-start justify-between gap-3">
                ...
                {openActionsBranchId === branch.id ? (
                  <div className="absolute right-4 top-14 z-20 w-52 rounded-2xl border border-border bg-white p-2 shadow-2xl">
                    <button
                      className="flex h-10 w-full items-center gap-2 rounded-xl px-3 text-left text-sm font-bold text-stone-700 transition hover:bg-stone-50 hover:text-primary"
