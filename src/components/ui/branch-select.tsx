...
type BranchSelectOption = BranchOption | { id: 'all'; name: string; slug: 'all'; active: true; isMain: false };
  ...
  return (
    <div ref={rootRef} className="relative min-w-0">
      <button
        type="button"
        className={cx(
          'group flex h-12 w-full min-w-[230px] items-center gap-3 rounded-2xl border border-teal-100 bg-white/95 ps-1.5 pe-3 text-start shadow-sm shadow-teal-50 outline-none ring-1 ring-transparent backdrop-blur transition',
      ...
      >
        <span className="flex size-9 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-teal-500 to-teal-700 text-white shadow-sm shadow-teal-200 transition group-active:scale-95">
        ...
        <ChevronDown
          className={cx(
            'size-4 shrink-0 text-teal-700 transition duration-200',
            open && 'rotate-180',
          )}
        />
      </button>
      ...
      {open ? (
        <div
          className="absolute end-0 z-40 mt-2 max-h-72 w-full min-w-[260px] overflow-y-auto rounded-2xl border border-teal-100 bg-white p-1.5 shadow-2xl shadow-stone-200/70"
          role="listbox"
        >
          {options.map((branch) => {
            ...
            return (
              <button
                ...
                className={cx(
                  'flex min-h-12 w-full items-center gap-3 rounded-xl px-2.5 py-2 text-start transition',
                  isSelected ? 'bg-teal-50 text-teal-900' : 'text-stone-700 hover:bg-stone-50 hover:text-stone-950',
                )}
              ...
              >
                ...
                <span className="min-w-0 flex-1">
                  ...
                  {!isAll ? (
                    <span className="mt-0.5 flex items-center gap-1.5 text-xs font-bold text-muted-foreground">
                      <span className={cx('size-1.5 rounded-full', branch.active ? 'bg-emerald-500' : 'bg-stone-300')} />
                      {branch.slug}
                    </span>
                  ) : null}
                </span>
