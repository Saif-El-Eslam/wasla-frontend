...
export function PublicItemModal({
  ...
  return (
    <div className="fixed inset-0 z-20 grid place-items-end bg-black/40 p-4 sm:place-items-center" onClick={onClose}>
        ...
        <div className="p-5">
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-2 text-sm leading-6 text-muted-foreground">{textForLocale(item.description, locale) || t('noDescriptionProvided')}</p>
            </div>
            <IconButton label={t('closeItem')} onClick={onClose}>
              <X className="size-4" />
            </IconButton>
          </div>
          <p className="mt-4 rounded-2xl bg-teal-50 px-4 py-3 text-sm font-black text-primary">{itemPriceText(item, currency, t('noPrice'))}</p>
        </div>
      </div>
    </div>
  );
